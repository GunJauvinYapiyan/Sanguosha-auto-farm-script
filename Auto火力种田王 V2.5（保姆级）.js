...
// ============================================================
var PANEL = {
    WHEAT_COOLDOWN_SEC: 63,   // 小麦冷却秒数（无酒状态的基准值，喝酒会自动再减3秒）—— 按你的要求维持54，时序计算未改
    RICE_COOLDOWN_SEC: 243,   // 水稻冷却秒数（无酒状态的基准值，喝酒会自动再减6秒）
    RICE_EVERY_N_CHICKEN: 4,  // 鸡场真正处理满几轮后种一次水稻（水稻轮距）
    FACTORY_FEED_PULLS: 3,    // 中间两个铡刀坊（鸡场）各自的拖拽次数，默认3，对应鸡饲料x4
...
// 爆仓挽救后，标记本轮/下一轮暂停鸡场马场气泡检测（残留未收割作物的泛白会误触发气泡判定）。
// 置 true：handleWarehouseFullDuringHarvest() 执行完毕时（不管是种植阶段还是收割阶段触发的挽救）。
// 清 false：harvestAll() 里 DriftGuard.finalCheck 返回 'ok' 时（说明这一轮收割干净、没再次爆仓）。
var bubbleDetectSuspended = false;
// ================= 悬浮控制/暂停状态 =================
// CTRL.paused          —— 当前是否处于暂停态（由暂停/播放键或"设置未关闭强制暂停"设置）
// CTRL.settingsOpen    —— 设置面板当前是否展开
// CTRL.actionInProgress—— true = 动作进行中，设置键锁死；false = 空闲，设置键可点
// CTRL.currentPhase    —— 'plant' | 'harvest'，仅用于记录暂停发生时"刚种"还是"刚收"（日志用），
//                         不再用于自动决定恢复流程——恢复动作固定由用户点播放键后在弹窗里手动选择
//                         "种植"或"收割"（见 handlePauseAndResume / showResumeChoiceDialog）
var CTRL = {
    paused: false,
    settingsOpen: false,
...
// 恢复流程用：值为 true 时，主循环跳过种植与卖货，直接从气泡检测开始，最后再收割一次
// —— 由用户在播放键弹窗里选择"收割"时设置（见 handlePauseAndResume）
var skipToHarvestOnce = false;
// 恢复流程用：暂停后，用户通过播放键弹出的选择弹窗决定的下一步动作（'plant' | 'harvest'）。
// 选"取消"则保持为 null，且 CTRL.paused 不变（继续保持暂停）。
var pendingResumeChoice = null;
// 用一个自定义异常类型标记"因暂停而中断"，和真正的运行时错误区分开
...
// 统一修改"动作进行中"标志，并同步悬浮条上设置键的视觉锁定状态
...
// ============================================================
//
// 阶段1 (softCheck)：种植后检测 → 嫌疑记录，不中断流程
// 阶段1.5 (onBubblesFound)：气泡检测到 = 画面没偏，解除嫌疑
// 阶段2 (midFlowRecheck)：鸡场/马场结束后补测 → 再给一次机会
// 阶段3 (finalCheck)：收割后硬校验 → 三次截图+交叉比对，全挂才判死
//
// 每次判死前保存全部调试截图到 /sdcard/wheat_farm_debug/
// ============================================================
var DriftGuard = {
    suspected: false,
    debugImgPaths: [],
    ...
    // 交叉比对：取首张和末张调试截图的参考区域互相匹配
    // 如果两张截图的同一区域长得一样 → 画面从没动过 → 是参考图老化
    crossCompare: function () {
        ...
        try {
            ...
            // 用0.80阈值（比正常宽松一点，因为是截图之间互比，可能有轻微渲染差异）
...
// ================= 菜单等待 =================
function clickAndAwaitMenu(clickX, clickY, checkX, checkY) {
    var imgBefore = safeCaptureScreen();
    var bgCol = null;
    if (imgBefore) {
        bgCol = images.pixel(imgBefore, checkX, checkY);
        imgBefore.recycle();
    }
    click(clickX, clickY);
    var maxWait = 3000;
    ...
    while (elapsed < maxWait) {
        ...
        if (elapsed >= 600) {
            ...
            if (imgNow) {
                var curCol = images.pixel(imgNow, checkX, checkY);
                imgNow.recycle();
                var diff = Math.max(
                    Math.abs(colors.red(bgCol) - colors.red(curCol)),
                    Math.abs(colors.green(bgCol) - colors.green(curCol)),
                    Math.abs(colors.blue(bgCol) - colors.blue(curCol))
                );
                if (diff > 20) {
                    changed = true;
                    break;
...
// ================= 四个主阶段 =================
// -------- 种植 --------
// 注意：currentPhase 和 lastPlantTime 在函数一开始就设置好，
// 哪怕手势画到一半被暂停打断，也按"这轮已经种下去了"处理（计数照常清零、冷却照常起算）。
// lastPlantTime 记录时机维持你v2.5原样（种之前记），配合 WHEAT_COOLDOWN_SEC=54，未做改动。
function plantAll() {
    CTRL.currentPhase = 'plant';
    var useRice = riceQueued;
    riceQueued = false;
    lastPlantTime = Date.now();
    var iconPos = useRice ? CONFIG.riceSeedIcon : CONFIG.seedIcon;
    clickAndAwaitMenu(CONFIG.targetTile[0], CONFIG.targetTile[1], iconPos[0], iconPos[1]);
    if (useRice) {
        toastLog("== 播种水稻 ==");
        dragFarmLoop("rice");
        isRiceRound = true;
...
// -------- 等冷却 --------
// actionInProgress 只在这段真正"没事可做"的冷却等待期间设为 false（设置键解锁），
// 等待一结束就立刻重新锁上，不会出现"解锁窗口被压缩成0秒"的问题。
function waitUntilFree() {
    ...
    // 即将开始收割前的动作了，如果设置面板还开着，强制走一次暂停流程（并真正把面板收起来）
    ...
    sleep(600);   // 改用原生sleep：紧贴"点地→点免费"操作，不能被暂停打断，否则后面拿镰刀会错位
    click(CONFIG.freeButtonPos[0], CONFIG.freeButtonPos[1]);
    sleep(500);   // 同上，改用原生sleep
}
// -------- 仓库已满挽救 --------
function handleWarehouseFullDuringHarvest() {
    ...
    pausableSleep(1500);
    skipNextSell = true;
    // 本轮收割没能真正把作物收进仓库（卡在满仓弹窗），地里大概率还留着上一轮成熟作物的泛白效果，
    // 先把鸡场/马场气泡检测停掉，等确认某一轮收割干净（没再弹满仓）以后再恢复。
    bubbleDetectSuspended = true;
    log('仓库已满挽救流程完成，已标记暂停气泡检测直到下次收割干净');
}
// -------- 收割 --------
// currentPhase 和 lastHarvestWasRice 同样在函数一开始就写好，
// 哪怕收割手势画到一半被暂停打断，也按"这轮已经收了"处理。
function harvestAll() {
    CTRL.currentPhase = 'harvest';
    lastHarvestWasRice = isRiceRound;
    clickAndAwaitMenu(CONFIG.targetTile[0], CONFIG.targetTile[1], CONFIG.sickleIcon[0], CONFIG.sickleIcon[1]);
...
// ================= 暂停恢复处理 =================
// 被 pausableSleep 抛出的 PauseSignal 会一路冒泡到主循环的 catch，在那里调用这个函数。
//
// 注意：游戏内部时间在脚本暂停期间是不会停的，所以不管暂停发生在"刚种"还是"刚收"，
// 都不再尝试去精确推算冷却剩余时间、也不再尝试自动点击"免费"按钮去猜时机——
// 暂停时间越长，这个猜测就越不可靠（免费按钮很可能已经不在了，或者作物早就能直接收割了）。
// 现在恢复动作固定由用户点击播放键时通过弹窗明确选择：种植 / 收割 / 取消，
// 两种情况都统一从基准地重新开始对应流程，不做任何"接着上次进度往下走"的尝试。
function handlePauseAndResume() {
    setActionInProgress(false);
    log('[暂停] 阶段=' + CTRL.currentPhase + '，等待用户通过播放键选择继续时的动作（种植/收割/取消）');
    // 阻塞等待，直到用户在悬浮条上选择了"种植"或"收割"（选"取消"则保持暂停，这个循环不会退出）
    while (CTRL.paused) {
        sleep(200);
        renewLock();
    }
    ...
    try {
        if (choice === 'harvest') {
            // 用户选择：下一步直接收割。跳过种植与卖货，从基准地开始气泡检测→收割
            toastLog('10秒后开始收割，请确保下方镰刀图标已出现');
            log('[恢复] 用户选择"收割"，等待10秒后直接收割');
            pausableSleep(10000);
            refreshDriftReference();
            skipNextSell = false; // 本轮不经过卖货环节，清掉可能残留的爆仓挽救跳过标记，避免误伤下一轮
            skipToHarvestOnce = true;
        } else {
            // 用户选择：下一步重新种植（choice 应为 'plant'；异常情况下也按种植处理更安全）
...
// 注意：这里不再调用 floaty.closeAll()。
// 旧版本在这里调用过 floaty.closeAll()，脚本启动前调用没问题，
// 但局内重新校准（鸡场/马场开关触发）时会把常驻悬浮控制条也一起关掉且不会重建，
// 这正是"设置鸡蛋坐标后悬浮条消失"的根因，现在只关自己创建的 marker/confirmWin。
...
// -------- 继续时的选择弹窗：种植 / 收割 / 取消 --------
// 由播放键触发，跑在独立线程里（避免阻塞悬浮窗UI线程），风格与其它开关的二次确认弹窗一致。
// 选"取消"（或返回键关闭）：维持暂停，什么都不做。
// 选"种植"或"收割"：把选择记到 pendingResumeChoice，并把 CTRL.paused 置为 false，
// 唤醒主线程里 handlePauseAndResume() 的等待循环，由它负责后续的10秒提示与状态设置。
...
function createControlPanel() {
    ...
    // -------- 暂停/播放：任何时候都可以点 --------
    // 暂停：立即生效，逻辑不变。
    // 继续：不再直接恢复，而是弹窗让用户明确选择接下来是"种植"还是"收割"（或"取消"维持暂停）——
    // 因为游戏内部时间在暂停期间不会停，脚本没办法可靠地推算出该接着上次进度做什么。
    ...
    ctrlWin.chickenTgl.click(function () {
        if (PANEL.CHICKEN_ENABLED) {
            ...
            threads.start(function () {
                if (!chickenCalibrated) {
                    // 需要先去局内定位鸡蛋坐标：强制暂停 + 收起设置面板，定位完成后由用户自行点播放继续
                    forceCloseSettingsPanel();
                    CTRL.paused = true;
                    ctrlWin.ppBtn.post(function () { ctrlWin.ppBtn.setText(" ▶ "); });
    ...
    // 面板初始状态（还没打开过设置时，先按当前 PANEL/potionActive 的真实值同步一次）
    syncSettingsPanelToState();
}
// ================= 启动流程 =================
try {
...
// 开关决定要不要弹校准；如果之前已经校准并保存过，直接复用，不再重复弹
if (PANEL.CHICKEN_ENABLED) {
    if (!chickenCalibrated) {
        CONFIG.eggSlotInWarehouse = calibrateAndVerify("打开仓库，把红十字拖到鸡蛋图标上，再点\"确定这里\"");
        CONFIG.eggSellBtn = calibrateAndVerify("点一下鸡蛋弹出详情，把红十字拖到\"出售\"按钮上，再点\"确定这里\"");
...
initImg.recycle();
// 创建常驻悬浮控制条（此后脚本绝不会再调用 floaty.closeAll，它会一直存活到脚本结束）
createControlPanel();
toastLog("记录完毕，开始挂机");
...
while (true) {
    try {
        ...
        if (skipToHarvestOnce) {
            ...
            setActionInProgress(true);
            // 先点安全位置，关掉可能已经手动打开的镰刀菜单，避免clickAndAwaitMenu点地时把菜单"点没了"
            click(CONFIG.SAFE_CLOSE[0], CONFIG.SAFE_CLOSE[1]);
            sleep(500);
            harvestAll();
        } else {
